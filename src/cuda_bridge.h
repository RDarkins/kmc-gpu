 ...
 * host memory, the GPU global memory, and the GPU SM registers.
 */
...
/**
 * Transfer lattice from the host to d_state (GPU glob mem).
...
void loadLattice(int* d_state, Lattice const& lattice, GridParams const& gp) {
  ...
  gpu_assert(cudaMemcpy(d_state, h_state.data(), h_state.size() * sizeof(int), cudaMemcpyHostToDevice));
}

/**
 * Transfer from d_state (GPU glob mem) back to lattice on the host.
 */
template<int tw>
void unloadLattice(Lattice& lattice, int* d_state, GridParams const& gp) {
  std::vector<int> h_state(lattice.volume);
  gpu_assert(cudaMemcpy(h_state.data(), d_state, h_state.size() * sizeof(int), cudaMemcpyDeviceToHost));
